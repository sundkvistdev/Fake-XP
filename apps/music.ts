...
export default function run(args: unknown, FCCF: IFCCF, XP_API: IKernel, VFS: IVirtualFileSystem) {
    ...
    const items = ['C:/Music/Track1.mp3', 'C:/Music/Track2.mp3'];
    const playlist = FCCF.Controls.List({
        items: items,
        onItemClick: (item: string) => {
            setTrack(item.split('/').pop() || item);
            setPlaying(true);
        }
    });
    ...
    const mainPane = FCCF.Controls.Pane({
        ...
        children: [
            ...
            controls,
            FCCF.Controls.Pane({ style: { marginTop: '20px', flexGrow: '1', overflow: 'auto' }, children: [playlist] })
        ]
    });
