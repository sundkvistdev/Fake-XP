...
export default function run(args: unknown, FCCF: IFCCF, XP_API: IKernel, VFS: IVirtualFileSystem) {
    ...
    const grid = FCCF.Controls.Grid({
        ...
        children: items.map(item => {
            ...
            pane.el.onclick = item.action || (() => XP_API.showDialog({ message: item.name + ' is not implemented yet.' }));
            pane.el.onmouseover = () => { pane.el.style.background = '#e5f3ff'; };
            pane.el.onmouseout = () => { pane.el.style.background = 'transparent'; };
            return pane;
        })
    });
