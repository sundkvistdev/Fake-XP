...
export default function run(args: unknown, FCCF: IFCCF, XP_API: IKernel, VFS: IVirtualFileSystem) {
    ...
    const titleText = document.createTextNode('CentralFirm Antivirus');
    const statusText = document.createTextNode('System Status: ' + getStatus());
    ...
    const header = FCCF.Controls.Pane({
        ...
        children: [
            ...
            FCCF.Controls.Pane({ children: [
                FCCF.Controls.Pane({ style: { fontSize: '24px', fontWeight: 'bold' }, children: [titleText] }),
                FCCF.Controls.Pane({ style: { opacity: '0.8' }, children: [statusText] })
            ]})
        ]
    });
    ...
    subscribeStatus((s) => {
        statusText.nodeValue = 'System Status: ' + s;
    });

    subscribeProgress((p) => {
        // Safe check for progress bar setter if it exists
        const pb = progressBar as unknown as { setProgress?: (v: number) => void };
        if (typeof pb.setProgress === 'function') {
            pb.setProgress(p);
        } else {
            // Otherwise adjust the css/attribute directly
            const fillEl = progressBar.el.querySelector('.progress-bar-fill') as HTMLElement;
            if (fillEl) {
